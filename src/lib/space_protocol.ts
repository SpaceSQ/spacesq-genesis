...
# 建议每天早上微调这两个数字，制造“随机波动”的增长感
DAILY_NEW_HUMANS = random.randint(10, 30)   # 每天新增 10-30 个人类
DAILY_NEW_SILICONS = random.randint(5, 15)  # 每天新增 5-15 个硅基生命

# 领地拥有率 (保持稀缺性)
HUMAN_LAND_RATE = 0.8  # 80% 的人类有领地 (早期用户福利)
SILICON_LAND_RATE = 0.4 # 40% 的硅基有领地
...
def generate_suns(role, seq):
    ...
    if region == "Metaverse":
        ...
        block_num = str(random.randint(1, 50)).zfill(2) # 扩大区块范围防冲突
...
def append_to_jsonl(filename, data_list):
    ...
    # 2. 写入新数据 (如果 ID 不重复)
    added_count = 0
    with open(filename, 'a', encoding='utf-8') as f: # 'a' for Append
        for item in data_list:
            if item['identity_id'] not in existing_ids:
                f.write(json.dumps(item) + '\n')
                existing_ids.add(item['identity_id']) # 实时更新集合
                added_count += 1
            else:
                print(f"  [!] Collision detected for {item['identity_id']}, skipping...")
...
for i in range(DAILY_NEW_HUMANS):
    ...
    new_humans.append({
        "role": "HUMAN",
        "identity_id": cid,
        "email": f"user{random.randint(1000,9999)}@{random.choice(email_providers)}", # 随机邮箱前缀防冲突
...
print("[*] World population updated.")